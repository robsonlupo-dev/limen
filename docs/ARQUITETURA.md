# Limen — Arquitetura e Features (referência detalhada)

> **Este arquivo é referência.** Saiu do `CLAUDE.md` no PR `docs/slim-claude-md`
> para o guia caber no limite de contexto. Contém a **pilha tecnológica detalhada**
> e a **descrição por feature/serviço** (regras, invariantes de privacidade,
> contratos de cada superfície). São detalhes que **não mudam entre tarefas** —
> consulte a seção relevante ao mexer na feature correspondente.
>
> - Regras operacionais vivas + invariantes de engenharia da economia → `CLAUDE.md`
> - Histórico cronológico de sprints → `docs/HISTORICO_SPRINTS.md`
> - Regras de negócio da economia (preços/splits/tiers) → `docs/ECONOMIA.md`
> - Decisões de produto de ago/2026 → `docs/DECISOES_2026-08.md`
> - Pendências jurídicas → `docs/PENDENCIAS_JURIDICAS.md`

## Índice
- **Pilha tecnológica (Stack)** — PHP/Laravel, MySQL, Redis, Inertia/Vue, Asaas/PIX, Reverb, LiveKit
- **Privacidade do membro** — decisões locked, piso de anonimato, piso de visitantes, k-anonimato
- **FanAlias** — pseudônimo por par membro↔performer
- **Favoritos / Notas da performer / Boost pago** — bookmark privado, anotação privada, destaque no catálogo
- **Convite via Stories / Buscas salvas / Filtro de cidade** — iscas e filtros do catálogo
- **Catálogo de membros / Chat economy v2 / Vitrine de conteúdo / Extrato de ganhos** — superfícies membro↔performer
- **Foto de perfil do membro / Visitas bidirecionais / Sinais de atividade / Teaser / Microinterações / Intro de voz** — fila de melhorias
- **Landing cinematográfica** — a porta pública `/` (cenas, movimento, waitlist)
- **Anti-CSAM / Som de notificação** — moderação e preferências
- **Agendamento de chamada / Console de live / Chamada privada a partir da live** — LiveKit
- **PanicButton / Navegação (painel performer e membro)** — saída rápida e nav mobile-first
- **Foto Efêmera / Stories da Performer** — conteúdo efêmero e moderação
- **2FA / Login OTP / Captcha / Geobloqueio / Filtro de chat / Aceite de documentos** — segurança e compliance
...
- **Ligar/desligar a CÂMERA sem encerrar** (`toggleCamera` → `setCameraEnabled(false/true)`
  ...
  **Convivência com a pausa por chamada privada (ver § abaixo):** os dois avisos do
  LiveViewer são estados DISTINTOS e ambos existem; se a sala estiver pausada por chamada
  privada, o aviso "Em chamada privada — volta já" tem PRECEDÊNCIA (o `v-else-if="paused"`
  vem antes do `v-else-if="cameraOff"`, então "câmera desligada" nem é avaliado enquanto a
  sala inteira está pausada).
