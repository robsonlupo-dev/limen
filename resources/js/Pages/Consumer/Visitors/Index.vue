...
/**
 * "Quem visitou seu perfil" — as PERFORMERS que visitaram o membro (visitas
 * bidirecionais, A.0.4).
 *
 * Assimetria com o painel de visitantes da performer: aqui é a PERFORMER que se
 * expõe, com a identidade pública do catálogo (nome artístico, avatar, slug) — sem
 * FanAlias, sem piso, sem paywall. Seguro por natureza: performer é pública, e
 * nenhum membro é exposto nesta tela. Nada de PII, nada de tier.
 */
defineProps({
    // [{ stage_name, slug, avatar_url, visited_slot }]
    performers: { type: Array, default: () => [] },
})
</script>

<template>
    <AppLayout title="Quem me visitou">
        <div class="bg-limen-bg">
            <div class="max-w-screen-2xl mx-auto px-6 py-10 space-y-8">
                <div class="space-y-2">
                    <h1 class="font-serif text-4xl text-limen-ink">Quem visitou seu perfil</h1>
                    <p class="text-sm text-limen-ink-soft">
                        Performers que passaram pelo seu perfil recentemente. Toque para conhecer.
                    </p>
                </div>
                ...
                <div v-if="performers.length === 0" class="flex flex-col items-center justify-center gap-4 py-24 text-center">
                    ...
                    <p class="font-serif text-2xl text-limen-ink">Ninguém visitou seu perfil ainda.</p>
                ...
                <div v-else class="grid grid-cols-2 gap-3.5 md:grid-cols-3 lg:grid-cols-4">
                    ...
                    >
                        ...
                        <div class="absolute inset-x-0 bottom-0 p-3">
                            ...
                            <p class="mt-0.5 text-[11px] text-limen-ink-mute">Visitou você · {{ performer.visited_slot }}</p>
                        </div>
                    </Link>
                </div>
            </div>
